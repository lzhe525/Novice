...
3. **解析要点**：
   ...
   - `onboarding-status.md`：`currentPhase` 与各布尔完成标志。
...
4. **按下列优先级**决定「推荐下一步 Skill」（命中即选，可辅以人类动作）：

| 优先级 | 条件（摘要） | 推荐 |
|--------|----------------|------|
| P1 | `onboarding-status.md` 不存在，或 `currentPhase` 未到 `phase_handoff_next` 且存在未完成的 Bootstrap 事实 | `skills/bootstrap/guide-project-onboarding.md` |
